...
void joinComps(int parents[], int rank[], int startOne, int startTwo, std::vector<int> groups[])
{
...
void minSpanningTree(std::vector<edge> graph[], std::vector<edge> result[], int root, int numVertices, int numEdges, bool notConnected[], int cores)
{
	...
	std::vector<int> *groups = new std::vector<int>[numVertices];
    ...
    while(numTrees > 1)
    {
		...
		*/
        ...
        for(int i = 1; i < numVertices; i++)
        {
            ...
            if(notConnected[i] || groups[i].back() == -1)
			{
				//debug
			...
			for(int j : groups[i])
			{
				for(auto curr : graph[j])
				{
					...
					int setOne = groups[i].front();//find(parents, rootOne);
